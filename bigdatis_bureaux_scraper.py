...
class BigdatisBureauxScraper:
    ...
    def process_annonce(self, annonce, page, type_name):
        try:
            ...
            return {
                ...
                "type_bureau": type_name,
                "type_bureau_code": properties.get("propertyType"),
    ...
    def save_data(self):
        ...
        filename = f"bigdatis_bureaux_{mode_suffix}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏢 BIGDATIS SCRAPER - BUREAUX")
    print("=" * 70)
    print("\nType de bien:")
    print("  • Bureau (office)")
    print("\nDonnées récupérées:")
    ...
    if mode_choice == "2":
        # Mode complet
        scraper = BigdatisBureauxScraper(test_mode=False)
        print("\n✅ MODE COMPLET sélectionné")
        print("⚠️  ATTENTION: Le scraping complet peut prendre du temps")
        print("   et va récupérer toutes les annonces de bureaux disponibles.")
        ...
        scraper = BigdatisBureauxScraper(test_mode=True, max_properties=20)
