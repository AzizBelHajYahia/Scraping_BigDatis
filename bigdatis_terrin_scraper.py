...
class BigdatisTerrainScraper:
    def __init__(self, test_mode=False, max_terrains=20):
        ...
        self.max_terrains = max_terrains if test_mode else float('inf')

        # Tous les types de terrains
        if test_mode:
            # En mode test, seulement terrain constructible pour aller plus vite
            self.property_types = {
                'buildingLot': 'Terrain constructible',
            }
        else:
    ...
    def scrape_property_type(self, property_type, type_name, fetch_details=True):
        mode_label = f"TEST - MAX {self.max_terrains}" if self.test_mode else "COMPLET"
        print("\n" + "=" * 70)
        print(f"🏞️  SCRAPING {mode_label}: {type_name.upper()}")
        if fetch_details:
        ...
        while len(data) < self.max_terrains:
            ...
            try:
                ...
                print(f"✅ {len(annonces)} terrains ({len(new_ids)} nouveaux)", end="")
                seen_ids.update(current_ids)

                for i, annonce in enumerate(annonces):
                    # Stop if we've reached the limit (only in test mode)
                    if len(data) >= self.max_terrains:
                        print(f"\n   🎯 LIMITE ATTEINTE: {int(self.max_terrains)} terrains")
                        break
                    ...
                    if annonce.get("id") in new_ids:
                        ...
                        if fetch_details:
                            if self.test_mode:
                                ...
                                print(f"\n   └─ [{len(data)+1}/{int(self.max_terrains)}] ID: {annonce.get('id')}", end="", flush=True)
                            ...
                            if not self.test_mode and (i + 1) % 10 == 0:
                                print(f"\n   └─ Progression: {len(data)+1} terrains traités", end="", flush=True)
                ...
                if self.test_mode:
                    print(f"\n   📊 Progression: {len(data)}/{int(self.max_terrains)} | Sources: {sources_count} ({sources_with_urls} avec URLs)")
                else:
        ...
        print(f"\n✅ {type_name}: {len(data):,} terrains en {elapsed:.1f}s")
        return data
    ...
    def process_annonce(self, annonce, page, type_name):
        try:
            ...
            return {
                ...
                "type_terrain": type_name,
                "type_terrain_code": properties.get("propertyType"),
            ...
            print(f"❌ Erreur terrain {annonce.get('id')}: {e}")
            import traceback
            traceback.print_exc()
            return None

    def scrape_all_terrains(self, fetch_details=True):
        for code, name in self.property_types.items():
            data = self.scrape_property_type(code, name, fetch_details)
            self.all_data.extend(data)
            print(f"📊 Total cumulé : {len(self.all_data):,} terrains")
            
            # Stop if we've reached the limit (only in test mode)
            if self.test_mode and len(self.all_data) >= self.max_terrains:
                break
    ...
    def save_data(self):
        ...
        filename = f"bigdatis_terrains_{mode_suffix}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        ...
        print("=" * 70)
        print(f"📁 {filename}")
        print(f"📊 Total terrains : {len(df):,}")
        
        # Show sample data in test mode
        if self.test_mode:
            print("\n📋 APERÇU DES DONNÉES (5 premiers terrains):")
            print("-" * 70)
        ...
        for terrain_type in df["type_terrain"].unique():
            count = len(df[df["type_terrain"] == terrain_type])
            avg_surface = df[df["type_terrain"] == terrain_type]["surface"].mean()
            avg_price = df[df["type_terrain"] == terrain_type]["prix"].mean()
            print(f"   • {terrain_type}: {count:,} terrains")
        ...
        print(f"   • Terrains avec sources : {with_sources:,} / {len(df):,} ({with_sources/len(df)*100:.1f}%)")
        print(f"   • Total sources : {int(total_sources):,}")
        print(f"   • Moyenne sources/terrain : {avg_sources:.2f}")
        if total_sources > 0:
            print(f"   • Sources avec URLs : {int(total_urls):,} ({total_urls/total_sources*100:.1f}% des sources)")
            print(f"   • Terrains avec URLs : {with_urls:,} / {len(df):,} ({with_urls/len(df)*100:.1f}%)")
        
        # Price statistics
        print(f"\n💰 Statistiques prix:")
        with_price = df[df["prix"].notna()]
        if len(with_price) > 0:
            print(f"   • Terrains avec prix : {len(with_price):,} / {len(df):,} ({len(with_price)/len(df)*100:.1f}%)")
        ...
        if with_contacts > 0:
            print(f"   • Terrains avec contacts : {with_contacts:,} / {len(df):,} ({with_contacts/len(df)*100:.1f}%)")
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏞️  BIGDATIS SCRAPER - TERRAINS AVEC SOURCES COMPLÈTES")
    print("=" * 70)
    print("\nTypes de terrains disponibles:")
    print("  • Terrain constructible (buildingLot)")
    print("  • Terrain agricole (farmland)")
    print("\nDonnées récupérées:")
    print("  ✓ Informations de base (titre, prix, surface, location)")
    ...
    print("=" * 70)
    print("1. Mode TEST - 20 terrains (rapide, pour tester)")
    print("2. Mode COMPLET - Tous les terrains (peut prendre plusieurs heures)")
    print()
    ...
    if mode_choice == "2":
        # Mode complet
        scraper = BigdatisTerrainScraper(test_mode=False)
        print("\n✅ MODE COMPLET sélectionné")
        print("⚠️  ATTENTION: Le scraping complet peut prendre plusieurs heures")
        print("   et va récupérer tous les terrains disponibles.")
        ...
        scraper = BigdatisTerrainScraper(test_mode=True, max_terrains=20)
        print("\n✅ MODE TEST sélectionné (20 terrains)")
    ...
    if response in ["", "oui", "o", "y", "yes"]:
        ...
        scraper.scrape_all_terrains(fetch_details=True)
        print(f"\n✅ Scraping {mode_label} terminé avec succès!")
        
        if scraper.test_mode:
            print("\n💡 Le test est réussi ! Pour scraper tous les terrains,")
