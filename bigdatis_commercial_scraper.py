...
class BigdatisCommercialScraper:
    def __init__(self, test_mode=False, max_properties=20):
        ...
        # Tous les types de locaux commerciaux et industriels
        if test_mode:
            # En mode test, seulement locaux commerciaux pour aller plus vite
            self.property_types = {
            ...
            self.property_types = {
                'commercialPremise': 'Local commercial',
                'industrialPremise': 'Local industriel',
    ...
    def save_data(self):
        ...
        filename = f"bigdatis_commercial_{mode_suffix}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        ...
        for local_type in df["type_local"].unique():
            count = len(df[df["type_local"] == local_type])
            avg_surface = df[df["type_local"] == local_type]["surface"].mean()
            avg_price = df[df["type_local"] == local_type]["prix"].mean()
            print(f"   • {local_type}: {count:,} annonces")
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏭 BIGDATIS SCRAPER - LOCAUX COMMERCIAUX & INDUSTRIELS")
    print("=" * 70)
    print("\nTypes de locaux disponibles:")
    print("  • Local commercial (commercialPremise)")
    print("  • Local industriel (industrialPremise)")
    print("  • Dépôt/Entrepôt (warehouse)")
    print("\nDonnées récupérées:")
    ...
    if mode_choice == "2":
        # Mode complet
        scraper = BigdatisCommercialScraper(test_mode=False)
        print("\n✅ MODE COMPLET sélectionné")
        print("⚠️  ATTENTION: Le scraping complet peut prendre plusieurs heures")
        print("   et va récupérer toutes les annonces commerciales/industrielles disponibles.")
        ...
        scraper = BigdatisCommercialScraper(test_mode=True, max_properties=20)
