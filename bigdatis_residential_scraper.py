...
class BigdatisResidentialScraper:
    def __init__(self, test_mode=False, max_properties=20):
        ...
        if test_mode:
            # En mode test, seulement appartements pour aller plus vite
            self.property_types = {
                'flat': 'Appartement',
            }
        else:
    ...
    def process_annonce(self, annonce, page, type_name):
        try:
            ...
            return {
                ...
                "typologie": properties.get("typology", "").upper(),
                "type_bien": type_name,
    ...
    def save_data(self):
        ...
        filename = f"bigdatis_residential_{mode_suffix}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        ...
        if self.test_mode:
            ...
            for idx, row in df.head(5).iterrows():
                ...
                if pd.notna(row['prix']) and pd.notna(row['surface']):
                    print(f"   Prix: {row['prix']:,.0f} DT | Surface: {row['surface']:,.0f} m² | Type: {row['typologie']}")
                else:
                    print(f"   Type: {row['typologie']} | Prix/Surface: N/A")
        ...
        for bien_type in df["type_bien"].unique():
            count = len(df[df["type_bien"] == bien_type])
            avg_surface = df[df["type_bien"] == bien_type]["surface"].mean()
            avg_price = df[df["type_bien"] == bien_type]["prix"].mean()
            print(f"   • {bien_type}: {count:,} annonces")
            if not pd.isna(avg_surface):
                print(f"     └─ Surface moyenne: {avg_surface:.0f} m²")
            if not pd.isna(avg_price):
                print(f"     └─ Prix moyen: {avg_price:,.0f} DT")
        
        # Statistics by typologie
        if "typologie" in df.columns:
            print("\n📊 Répartition par typologie:")
            typo_counts = df["typologie"].value_counts().head(10)
            for typo, count in typo_counts.items():
                if typo:  # Skip empty values
                    avg_price = df[df["typologie"] == typo]["prix"].mean()
                    print(f"   • {typo}: {count:,} annonces", end="")
                    if not pd.isna(avg_price):
                        print(f" (prix moyen: {avg_price:,.0f} DT)")
                    else:
                        print()
...
if __name__ == "__main__":
    print("=" * 70)
    print("🏠 BIGDATIS SCRAPER - PROPRIÉTÉS RÉSIDENTIELLES")
    print("=" * 70)
    print("\nTypes de biens disponibles:")
    print("  • Appartement (flat)")
    print("  • Maison/Villa (house)")
    print("  • RDC/Étage de villa (villaFloor)")
    print("  • Duplex/Triplex (duplex)")
    print("\nDonnées récupérées:")
    print("  ✓ Informations de base (titre, prix, surface, typologie)")
    ...
    print("2. Mode COMPLET - Toutes les annonces (peut prendre plusieurs heures)")
    print()
    ...
    if mode_choice == "2":
        # Mode complet
        scraper = BigdatisResidentialScraper(test_mode=False)
        print("\n✅ MODE COMPLET sélectionné")
        print("⚠️  ATTENTION: Le scraping complet peut prendre plusieurs heures")
        print("   et va récupérer toutes les annonces résidentielles disponibles.")
        ...
        scraper = BigdatisResidentialScraper(test_mode=True, max_properties=20)
